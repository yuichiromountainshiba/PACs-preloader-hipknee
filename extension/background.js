...
async function getFiltersFromStorage() {
  ...
  if (saved.lastFilters) return saved.lastFilters;
  // Default: all regions for this subspecialty so non-matching studies are filtered out
  const defaultRegions = Object.keys(SUBSPECIALTY.regionKeywords);
  return { regions: defaultRegions.length > 0 ? defaultRegions : null, modalities: ['xr', 'ct', 'mr'] };
}
